 ...
 * Compiled Fri, 16 Jul 2021 08:59:08 UTC
 *
...
var Particle = /** @class */ (function (_super) {
    ...
    Particle.prototype.update = function (delta) {
        ...
        if (this._doAlpha) {
            if (this.alphaYoyo) {
                var targetLerp = lerp * 2;
                if (targetLerp > 1) {
                    targetLerp = 2 - targetLerp;
                }
                this.alpha = this.alphaList.interpolate(targetLerp);
            }
            else {
 ...
 * A particle emitter.
 */
var EmitterEvent = Object.freeze({
    Spawn: 'spawn',
});
var Emitter = /** @class */ (function () {
    ...
    Emitter.prototype.init = function (art, config) {
        ...
        if (config.alphaYoyo) {
            this.alphaYoyo = config.alphaYoyo;
        }
        else {
            this.alphaYoyo = false;
        }
        ...
        if (config.colors) {
            ...
            for (var i = 0; i < config.colors.length; i++) {
                var node = PropertyNode.createList(config.colors[i]);
                this.startColors.push(node);
            }
        }
            ...
            while (this._spawnTimer <= 0) {
                ...
                if (-this._spawnTimer < lifetime) {
                    ...
                    for (var len = Math.min(this.particlesPerWave, this.maxParticles - this.particleCount); i < len; ++i) {
                        ...
                        if (this.startColors) {
                            var randomNum = Math.floor(Math.random() * (this.startColors.length));
                            p.colorList.reset(this.startColors[randomNum]);
                        }
                        else {
                        ...
                        this.eventEmitter.emit(EmitterEvent.Spawn, p); // fire spawn event
                    }
                }
    ...
    Emitter.prototype._spawnRect = function (p, emitPosX, emitPosY) {
        ...
        helperPoint$1.x = (Math.random() * this.spawnRect.width) + this.spawnRect.x;
        helperPoint$1.y = (Math.random() * this.spawnRect.height) + this.spawnRect.y;
        if (this.rotation !== 0) {
            exports.ParticleUtils.rotatePoint(this.rotation, helperPoint$1);
        }
        p.position.x = emitPosX + helperPoint$1.x;
        p.position.y = emitPosY + helperPoint$1.y;
    };
    /**
    ...
    Emitter.prototype._spawnCircle = function (p, emitPosX, emitPosY) {
        ...
        helperPoint$1.x = Math.random() * this.spawnCircle.radius;
        helperPoint$1.y = 0;
        // rotate the point to a random angle in the circle
        exports.ParticleUtils.rotatePoint(Math.random() * 360, helperPoint$1);
        // offset by the circle's center
        helperPoint$1.x += this.spawnCircle.x;
        helperPoint$1.y += this.spawnCircle.y;
    ...
    Emitter.prototype._spawnRing = function (p, emitPosX, emitPosY) {
        ...
        if (spawnCircle.minRadius !== spawnCircle.radius) {
            helperPoint$1.x = (Math.random() * (spawnCircle.radius - spawnCircle.minRadius))
                + spawnCircle.minRadius;
        }
        else {
            helperPoint$1.x = spawnCircle.radius;
        }
        helperPoint$1.y = 0;
        // rotate the point to a random angle in the circle
        var angle = Math.random() * 360;
        p.rotation += angle;
        exports.ParticleUtils.rotatePoint(angle, helperPoint$1);
        // offset by the circle's center
        helperPoint$1.x += this.spawnCircle.x;
        helperPoint$1.y += this.spawnCircle.y;
        // rotate the point by the emitter's rotation
    ...
    Emitter.prototype._spawnPolygonalChain = function (p, emitPosX, emitPosY) {
        ...
        this.spawnPolygonalChain.getRandomPoint(helperPoint$1);
        // rotate the point by the emitter's rotation
        if (this.rotation !== 0) {
            exports.ParticleUtils.rotatePoint(this.rotation, helperPoint$1);
        }
...
var PathParticle = /** @class */ (function (_super) {
    ...
    PathParticle.prototype.update = function (delta) {
        ...
        if (lerp >= 0 && this.path) {
            ...
            exports.ParticleUtils.rotatePoint(this.initialRotation, helperPoint);
