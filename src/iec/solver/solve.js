...
export class Solver {
  ...
  isLegalMove(orig, dest) {
    ...
    if (this.stacks[dest].full()) {
      return false;
    }

    if (this.stacks[orig].empty() || this.stacks[orig].solved()) {
      return false;
    ...
    if (this.stacks[dest].empty()) {
      ...
      return !this.stacks[orig].monocolor();

    }

    // Not illegal, but moving monocolor 3-stack makes no progress.

    if (this.stacks[orig].missingOne()) {
      return false;

    }
    if (this.stacks[orig].top() !== this.stacks[dest].top()) {
      return false;
  ...
  getKey() {
    ...
    return Solver.stacksToKey(this.stacks);
    ...
    while (stack.length > 0) {
      ...
      for (const move of current_state.getLegalMoves()) {
        ...
        if (!visited.has(new_state.getKey())) {
          ...
          parent[new_state.getKey()] = {};
          parent[new_state.getKey()].move = [move[0], move[1]];
          parent[new_state.getKey()].key = current_state.getKey();
